# The Non Profit Universal Basic Income Community 

The Universal Basic Income Community (UBIC) protocol is the DAO-based implemention of Universal Basic Income (UBI).

Example: **https://www.swiftdemand.com/**

## Abstract
The Universal Basic Income Community (UBIC) is the Decentralized Autonomous Organization (DAO) with Artificial Intelligence (AI) eventually providing the UBI through UBIC programs, blockchains (forked from the SwiftDemand blockchain that forked from NEO blockchain) and Universal Basic Income Tokens (UBITs), the electronic cryptocurrency used for all transactions on the UBIC mainnet that will be designed with [low transaction latency](#transaction-latency), the ability to [scale to ever-ascending transactions per second](#ever-ascending-transactions-per-second;TPS), [zero transaction fees](#0-transaction-fees), and [dispute resolution for disputed transactions](#buyer-seller-protections). UBI is distributed on a daily basis to each Citizen. This is the simplest understanding of how this Protocol works.
...
## Introduction
The Universal Basic Income Community (UBIC), Universal Basic Income (UBI) and the Universal Basic Income Token (UBIT) must exist for humans and their world to continue to function as jobs continue to be replaced by AI and automation <sup>[[1]](#1)</sup>. In a world with more abundant resources than ever before, many still struggle to make ends meet, and it is a primary directive UBIC to ensure every human has access to the core necessities of life that inlcudes water, food, clothing and shelter. This Protocol is the framework for the AI DAO purposed for distributing UBI to each Citizen through the UBIC double helix crossblockchains, website, platform, programs and UBITs. The mass adoption of this Protocol will allow the dream of the Universal Basic Income to become realized by each Citizen of UBIC. This protocol has been designed with practical and proven solutions, with additional functionality to aid in its global adoption.

## Account Types
To create the global electronic cryptocurrency capable of providing UBI to each Citizen, the following structure is established:
...
### The UBIC Account
The Universal Basic Income Community's account is the initial repository of all Universal Basic Income Tokens (UBITs) generated by UBIC's, from which UBITs may be distributed from time to time, to each Citizen.
...
### Incubation Period
All new Citizens that join the UBIC Protocol will be placed into an incubation period of one week. This prevents an Identity Provider from creating fake Citizens to quickly create UBITs for themselves--and gives Delegated Nodes sufficient time to ban the offending Identity Provider.
